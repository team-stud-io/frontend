...
import type { TodoDetail } from '../../services/home/homeService';

interface TaskDetailBottomSheetProps { difficulty: Difficulty | null; visible: boolean; detail?: TodoDetail | null; onClose: () => void; }
const STRATEGIES = [
...
export function TaskDetailBottomSheet({ difficulty, visible, detail, onClose }: TaskDetailBottomSheetProps) {
  ...
  return <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}><View style={styles.scrim}><View style={[styles.sheet, { paddingBottom: bottom }]}>
    ...
    <View style={styles.dividerBlock} /><View style={styles.recommendation}><View style={styles.recommendationTag}><Ionicons name="volume-high-outline" size={18} color="#5299A4" /><Text style={styles.recommendationText}>{tutorName} 튜터의 추천 공부법</Text></View><Text style={styles.duration}>권장 시간 · {duration}</Text></View>
    <ScrollView style={styles.strategyList} contentContainerStyle={styles.strategyContent} showsVerticalScrollIndicator={false}>{strategies.map(([title, body], index) => <View key={`${title}-${index}`} style={styles.strategyItem}><View style={styles.strategyNumber}><Text style={styles.strategyNumberText}>{index + 1}</Text></View><View style={styles.strategyCopy}><Text style={styles.strategyTitle}>{title}</Text><Text style={styles.strategyBody}>{body}</Text></View></View>)}</ScrollView>
...
const styles = StyleSheet.create({
  ...
  note: { position: 'absolute', right: 16, bottom: 10, left: 16, alignItems: 'center', paddingVertical: 4, borderWidth: 1, borderRadius: 6, borderColor: Colors['Line.Primary.Normal'], backgroundColor: Colors['Fill.Primary.Assistive'] }, noteText: { color: '#5299A4', fontFamily: 'Pretendard-Medium', fontSize: 12, lineHeight: 18 }, closeButton: { height: 60, alignItems: 'center', justifyContent: 'center', marginTop: 10, marginHorizontal: 20, borderWidth: 1, borderRadius: 16, borderColor: Colors['Line.Normal.Strong'], backgroundColor: '#F6F8FA' }, closeButtonText: { color: '#242628', fontFamily: 'Pretendard-SemiBold', fontSize: 17, lineHeight: 24 },
});
